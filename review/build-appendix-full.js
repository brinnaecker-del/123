...
push(TBL(['年份','总量口径泰尔指数（74区县）','剔除城关区后泰尔指数（73区县）','总量—人均秩相关系数（户籍口径）','总量—人均秩相关系数（常住口径）'],
  AU.A4.slice(1).map(r=>[r[0],r[1],r[2],r[3], r[0]==='2016'?'0.344':(r[0]==='2024'?'0.178':'【待补】')]),
  [1200,3400,3400,3300,3300],{numFrom:1}));
push(Note('注：泰尔指数采用份额熵形式 T=Σ(GDP_i/GDP)×ln[(GDP_i/GDP)×n]（n为区县数）。'
 ...
 +'秩相关系数为各区县GDP总量排名与人均GDP排名的斯皮尔曼秩相关系数，两种人口口径下均呈下降趋势。'
 +'正文§5(四)所引0.344（2016年）与0.178（2024年）为常住口径值，其余年份待补。'
 +'2024年城关区按户籍口径人均GDP在74个区县中排名第5位，常住口径下排名第14位。'
